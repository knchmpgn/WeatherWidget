    ...
    public partial class TaskbarWidget : Window
    {
        ...
        private void EnsureTopmost()
        {
            try
            {
                ...
                // This window is independent (not a child of Shell_TrayWnd).
                // Reassert HWND_TOPMOST to stay above the taskbar.
                // Do NOT check IsWindowVisible on the taskbar — during flyout transitions
                // the taskbar may momentarily report as hidden, and collapsing the widget
                // would create the very blink we are trying to prevent.
        ...
        private void PositionWidget()
        {
            ...
            if (!GetWindowRect(tray, out RECT trayRect) || !GetWindowRect(taskbar, out RECT tbRect)) return;
            ...
            switch (_taskbarEdge)
            {
                case ABE_BOTTOM:
                {
                    ...
                    screenX = trayLeft - this.ActualWidth - 8;
                    screenY = (tbRect.Top / dpi.DpiScaleY) + ((tbHeight - this.ActualHeight) / 2);
                    break;
                }
                case ABE_TOP:
                {
                ...
                case ABE_LEFT:
                {
                    ...
                    screenX = (tbRect.Left / dpi.DpiScaleX) + ((tbWidth - this.ActualWidth) / 2);
                    screenY = trayTop - this.ActualHeight - 8;
                    break;
                }
                case ABE_RIGHT:
                {
            ...
            // Since this window is no longer a child of Shell_TrayWnd, all coordinates are
            // in absolute screen space. No parent-relative conversion needed.
            this.Left = screenX;
            this.Top = screenY;
        }
        ...
        private bool IsFullscreenApplicationActive()
        {
            try
            {
                ...
                // Use a 98% coverage threshold: maximised windows with invisible borders
                // may not perfectly cover the monitor, but 98% is unambiguous.
                return winW >= monW * 0.98 && winH >= monH * 0.98;
            }
            catch
            {
                return false;
        ...
        private void PinWidgetToTaskbarZOrder(bool forceShow)
        {
            ...
            // This window is independent (not a child of Shell_TrayWnd).
            // Use HWND_TOPMOST to keep it above normal windows and the taskbar.
            // SWP_SHOWWINDOW is omitted here because ShowWindow (called above when
            // forceShow=true) already makes the window visible; including it would
            // trigger an unnecessary DWM repaint.
            SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, 0, 0,
                SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
        }
            ...
            IntPtr hwnd, int idObject, int idChild, uint dwEventThread, uint dwmsEventTime)
        {
            try
            {
                ...
                // ── Taskbar position change ──────────────────────────────────────────────
                // When the taskbar itself moves or resizes (e.g. taskbar unlock/drag,
                // auto-hide reveals, DPI change), immediately re-detect and reposition.
                ...
                // ── Coalesced z-order reassertion ────────────────────────────────────────
                // Rapid-fire WinEvents (ACTIVATE, SHOW, REORDER, CREATE, DESTROY) all arrive
                // during the same shell transition. Use a volatile flag to coalesce them into
                // a single SetWindowPos call at DispatcherPriority.Send, avoiding multiple
                // DWM repaints that would otherwise cause visible flicker.
